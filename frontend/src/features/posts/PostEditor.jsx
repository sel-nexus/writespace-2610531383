...
export default function PostEditor() {
  ...
  useEffect(() => {
    ...
    getPost(id, token)
      ...
      .catch((error) => {
        setRequestError(error.message || 'Unable to load this post.');
        setPhase('error');
      });
  }, [id, isEditing, token]);
      ...
      <section className="editor-card" aria-labelledby="editor-title">
        ...
        {phase !== 'loading' && phase !== 'error' && (
          <form className="post-form" onSubmit={submit} noValidate>
            ...
            <button className="primary-button" type="submit" disabled={phase === 'saving'}>
              {phase === 'saving' ? 'Saving…' : isEditing ? 'Save changes' : 'Publish post'}
            </button>
          </form>
        )}
      </section>
